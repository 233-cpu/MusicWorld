...
(function($) {
	$.fn.fullpage = function(options) {
		...
		// 自行添加的方法s
		$(document).on('click touchstart', '.leadA', function(e){
					e.preventDefault();
					var index = $(this).parent().index();
					scrollPage($('.fp-section').eq(index+1));
					
		});
		$(document).on('click touchstart','.index_link', function(e){
					e.preventDefault();
					var index = $(this).parent().index();
					scrollPage($('.fp-section').eq(index+1));
		...
		*/
		function scrollPage(element, callback, isMovementUp){
			...
			// 设置导航条样式动态变化
			$('.leadA').removeClass('activeLead')// 自行添加
			$($(".leadA")[v.sectionIndex-1]).addClass('activeLead')// 自行添加
